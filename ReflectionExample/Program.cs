...
namespace ReflectionExample
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            // ewaluacja kodu csharp w czasie rzeczywisty,
        ...
        static void CreateObject()
        {
            ...
            //zmiana wartosci dla pola
            PropertyInfo property = type.GetProperty("fieldB");
            if (property != null)
            {
                property.SetValue(castedClass, "ABCDEFGHIJKL");
                string newValue = property.GetValue(castedClass).ToString();
                Console.WriteLine(newValue);
            }

            //wywolanie metody przy pomocy refleksji
            MethodInfo method = type.GetMethod("MethodStr");
            if (method != null)
            {
                string result = method.Invoke(castedClass, new object[0]).ToString();
                Console.WriteLine(result);
            }
